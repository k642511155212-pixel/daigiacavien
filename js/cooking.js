(function () {
  ...
  class PrepManager {
    constructor(foods, modifiers, callbacks) {
      this.modifiers = modifiers;
      this.callbacks = callbacks || {};
      this.slots = foods.map(function (food, index) {
        return { index:index, station:food.station, stationName:food.stationName, state:"empty", foodId:null, elapsed:0, progress:0 };
      });
    }

    addFood(foodId) {
      const food = CVVH.Config.foodById(foodId);
      const slot = food && this.slots.find(function (item) { return item.station === food.station; });
      if (!slot || slot.state !== "empty") return null;
      slot.foodId = foodId; slot.state = "cooking"; slot.elapsed = 0; slot.progress = 0;
      return slot;
    }
    ...
    update(deltaMs) {
      this.slots.forEach((slot) => {
        ...
        if (slot.state === "cooking") {
          const prepTime = food.cookTime * this.modifiers.cookTimeScale;
          slot.progress = Math.min(1, slot.elapsed / prepTime);
          if (slot.elapsed >= prepTime) {
            slot.state = "ready"; slot.elapsed = 0; slot.progress = 0;
            if (this.callbacks.ready) this.callbacks.ready(slot);
          }
        } else if (slot.state === "ready") {
          const holdTime = food.burnTime * this.modifiers.readyTimeScale;
          slot.progress = Math.min(1, slot.elapsed / holdTime);
          if (slot.elapsed >= holdTime) {
            slot.state = "burnt"; slot.elapsed = holdTime; slot.progress = 1;
            if (this.callbacks.burnt) this.callbacks.burnt(slot);
          }
        }
      });
    ...
    collect(index) {
      const slot = this.slots[index];
      if (!slot || slot.state !== "ready") return null;
      const foodId = slot.foodId; this.clear(index); return foodId;
    }
    discardBurnt(index) {
      const slot = this.slots[index];
      if (!slot || slot.state !== "burnt") return false;
      this.clear(index); return true;
    }
    clear(index) {
      const slot = this.slots[index];
      if (!slot) return;
    ...
    isBusy(station) { const slot = this.slots.find(function (item) { return item.station === station; }); return !slot || slot.state !== "empty"; }
  }

  CVVH.Cooking = { FryerManager:FryerManager, PrepManager:PrepManager };
})();
