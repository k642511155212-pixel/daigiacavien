...
assert.equal(G.Characters.all.length, 27, "Updated recurring cast count is incorrect");
for (const removed of ["tram","my","gia-huy","co-thao","co-mai","thay-khoi"]) assert.equal(G.Characters.getById(removed), undefined, `Removed character still active: ${removed}`);
for (const character of G.Characters.all) assert.ok(character.dialogue.normal.length >= 3, `${character.name} needs varied normal dialogue`);
...
assert.ok(G.Characters.getById("duong").ageRole.includes("IT"), "Dương must study IT");
...
for (const id of ["phuoc-nguyen","anh-quan","thien-an","hoang-linh"]) assert.ok(G.Characters.getById(id), `Missing new character ${id}`);
assert.ok(G.Characters.getById("an").ageRole.includes("Nam sinh đội bóng rổ"), "An must be the new male basketball player");
assert.ok(G.CharacterSystem.knows("phuoc-nguyen","anh-quan"), "Phước Nguyên and Anh Quân must be best friends");
assert.ok(G.Characters.getById("thay-phong"), "Thầy Phong must remain in the cast");
assert.ok(G.Characters.getById("khanh").portionSizeWeight === "huge", "Khánh must favor huge randomized portions");
...
assert.ok(source["css/main.css"].includes('"Segoe UI"') && source["css/main.css"].includes("Arial"), "Vietnamese-safe system fallback stack is missing");
assert.ok(html.includes("KHÁNH BÉO ĐANG ĐỔ BỘ"), "Khánh warning overlay is missing");
assert.ok(html.includes("homi-mascot"), "Homi mascot is missing");
assert.ok(source["css/game.css"].includes("3.5s linear 2"), "Khánh marquee must complete each full-screen pass in 3.5 seconds without reversing");

const defaults = G.Storage.createDefaultSave();
assert.equal(defaults.money, 300000, "Starting capital must be 300,000 VND");
assert.equal(defaults.saveVersion, 4, "Save schema version 4 is missing");
...
assert.equal(migrated.saveVersion, 4, "Old save was not migrated to schema 4");
...
for (const character of G.Characters.all) {
  ...
  if (character.id === "khanh") assert.ok(Math.min(...sizes) >= 3, "Khánh's large randomized portion logic failed");
  assert.ok(simulationSave.recentOrders[character.id].length <= 5, `${character.name} recent-order history exceeded five entries`);
}

for (const day of [1,2,3,4,5,7,10,14]) {
  const available = G.Config.getUnlockedFoods(day);
  for (let i = 0; i < 1000; i += 1) {
    const order = G.Orders.generate(day, available, 8, seededRandom, false);
    assert.equal(G.Orders.validate(order, day), true, `Locked product leaked into an order on day ${day}`);
  }
}
assert.equal(G.Config.dailyCustomerTarget(1),8);
assert.equal(G.Config.dailyCustomerTarget(2),10);
assert.equal(G.Config.dailyCustomerTarget(3),12);
assert.equal(G.Config.dailyCustomerTarget(4),14);
assert.equal(G.Config.dailyCustomerTarget(5),16);
assert.ok(G.Config.dailyCustomerTarget(9)>=16 && G.Config.dailyCustomerTarget(9)<=18);
...
G.UI = {
  ...
  showKhanhAlert(done){ khanhAlerts += 1; done(); },
...
assert.equal(game.dailyTarget, 8, "Day 1 customer target should be eight");
...
const khanhSave = G.Storage.createDefaultSave();
khanhSave.tutorialCompleted = true;
khanhSave.story.introSeen = true;
khanhSave.story.seenScenes = ["intro"];
const khanhGame = new G.Game(khanhSave, {});
khanhGame.start(2);
khanhGame.customers = [];
khanhGame.spawnedCount = khanhGame.khanhSlot;
khanhGame.spawnCustomer(false);
assert.ok(khanhAlerts >= 1, "Khánh invasion alert did not trigger on guaranteed Day 2");
assert.ok(khanhGame.customers.some(function (customer) { return customer.characterId === "khanh" && customer.specialKhanh; }), "Khánh did not appear immediately after invasion alert");
